...
local middleware = require("bin.scaffold.build.middleware")
local project = require("bin.scaffold.build.project")
local app = require("bin.scaffold.build.app")
local Server = {}

function Server.nginx_conf_content()
...
function new_handler()
    ...
    local env = ogetenv("FW_ENV") or 'dev'
    return ngx_handle.new(
        necessary_dirs,
        Server.nginx_conf_content(),
        "conf/nginx-" .. env .. ".conf"
    )
end
...
function Server.project(name)
    project.new(name)
end


function Server.app(name)
    app.new(name)
end

function Server.middleware(project_name, middleware_name)
    middleware.new(project_name, middleware_name)
end
return Server
